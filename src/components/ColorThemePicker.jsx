...
export default function ColorThemePicker({ onThemeChange }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {/* Main button */}
      <motion.button
        className="p-4 rounded-full bg-white/20 backdrop-blur-lg shadow-lg border border-white/30 hover:scale-110 transition"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute bottom-16 right-0 bg-white/10 backdrop-blur-xl rounded-xl p-3 border border-white/20 shadow-xl"
          >
            <div className="flex flex-col gap-2">
              {gradients.map((g) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  aria-label={`Switch to ${g.id} theme`}
                />
              ))}
            </div>
          </motion.div>
